...
async def message_edit(event: hikari.GuildMessageUpdateEvent) -> None:
    ...
    if event.author.is_bot:
        return
    moderation_channel = fb.db.child("guilds").child(event.guild_id).child("moderation_channel").get().val()
    embed = hikari.Embed(
        title="Message has been edited by:", description=f'{event.author.mention}\n\n'
                                                         f'**Old message content:**\n{old.content}\n'
                                                         f'\n**New message content:**\n{event.message.content}\n\n'
                                                         f'**In channel:**\n'
                                                         f'{event.get_channel().mention}',
        timestamp=event.message.edited_timestamp,
        color=0xFCD203).set_thumbnail(event.author.avatar_url).set_footer(
        text=(me := plugin.bot.get_me()).username,
        icon=me.avatar_url)
...
async def message_delete(event: hikari.GuildMessageDeleteEvent):
    ...
    if (message := event.old_message) is None:
        await plugin.bot.rest.create_message(moderation_channel,
    ...
    embed = hikari.Embed(
        title="Message has been deleted by:", description=f'{deleter.mention}\n\n'
                                                          f'**Deleted message content:**\n{message.content}\n'
                                                          f'\n**In channel:**\n'
                                                          f'{event.get_channel().mention}',
        timestamp=message.timestamp,
        color=0xD9133C).set_thumbnail(deleter.avatar_url).set_footer(
        text=(me := plugin.bot.get_me()).username,
        icon=me.avatar_url)
    await plugin.bot.rest.create_message(moderation_channel, embed=embed)
