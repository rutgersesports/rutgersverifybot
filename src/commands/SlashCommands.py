...
@lightbulb.command(
    name="setagreementchannel", description="Sets the agreement channel of the server."
)
...
async def setAgreementChannel(ctx: lightbulb.SlashContext) -> None:
    ...
    if current == ctx.channel_id:
        ...
        return
    db.child("guilds").child(f"{ctx.guild_id}").child("agreementChannel").set(
        ctx.channel_id
    )
    await ctx.respond(
...
@lightbulb.option(
    name="role5",
    description="The role to add to this server's agreement roles.",
    type=hikari.Role,
    required=False,
)
@lightbulb.option(
...
@lightbulb.command(
    name="setnetidroles", description="Sets the agreement roles that require NetID."
)
@lightbulb.implements(lightbulb.SlashCommand)
